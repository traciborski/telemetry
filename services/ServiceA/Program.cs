...
builder.Services.AddSingleton(sp =>
    new KafkaProducer<OrderCreatedMessage>(bootstrapServers, sp.GetRequiredService<ILogger<KafkaProducer<OrderCreatedMessage>>>()));
...
app.MapPost("/orders", async (
    ...
    KafkaProducer<OrderCreatedMessage> producer,
    ...
    CancellationToken cancellationToken) =>
{
    ...
    logger.LogInformation(
        "Received order request for {Product} x{Quantity}, assigned OrderId {OrderId}",
        request.Product, request.Quantity, message.OrderId);
